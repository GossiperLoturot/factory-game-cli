...
Pipe::Pipe(Point begin, Point end, Anchor* begin_anchor, Anchor* end_anchor) : begin(begin), end(end), begin_anchor(begin_anchor), end_anchor(end_anchor) {}
...
void Pipe::Draw(DrawManager* draw_manager) {
	...
	{
		draw_manager->DrawHVLine(begin.x, begin.y, end.x, end.y);
	}
	else
	{
		draw_manager->DrawHVLine(begin.x, begin.y, begin.x, end.y);
		draw_manager->DrawHVLine(begin.x, end.y, end.x, end.y);
	}
}

void Pipe::CreateIdx(size_t* buffer, size_t self_idx) {
	// 垂直パイプ
	if (begin.x == end.x) {
		int x = begin.x;
...
void PipeManager::AddPipe(shared_ptr<Pipe> pipe)
{
	m_pipes.push_back(pipe);
}

// Immediaelyでバッファに書き込んでいる、データ構造を考えたい
shared_ptr<Pipe> PipeManager::RemovePipe(Point point)
{
	...
	for (size_t i = 0; i < m_pipes.size(); ++i) {
		shared_ptr<Pipe> pipe = m_pipes[i];
		pipe->CreateIdx(m_field.data(), i);
	}

	size_t index_to_remove = m_field[point.y * 120 + point.x];

	if (index_to_remove != static_cast<size_t>(-1)) {
		shared_ptr<Pipe> pipe = m_pipes[index_to_remove];
...
void PipeManager::RemovePipe(shared_ptr<Pipe> pipe)
{
