...
std::atomic_bool ok_winsize(false);

constexpr SHORT MIN_WIDTH = 120;
constexpr SHORT MIN_HEIGHT = 30;
...
int main() {
  ...
  SetConsoleMode(stdin_handle, (in_mode | ENABLE_WINDOW_INPUT |
  ...
  do {
    if (ok_winsize) {
      ...
      msg_stream << "Please resize the window least at " << MIN_WIDTH << "x"
                 << MIN_HEIGHT << ".";
      std::string msg = msg_stream.str();

      draw_manager.clear();
      draw_manager.draw_label(0, 0, msg);
